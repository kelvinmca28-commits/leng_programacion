...
DIFICULTADES = {
    "Fácil": (5, 10, 10, 1),      # Rango extendido a 20
    "Medio": (10, 20, 30, 1),     # Rango extendido a 150
    "Difícil": (15, 20, 50, 1),   # Rango extendido a 500 (Permite resultados negativos)
    "Experto": (20, 50, 100, 1)   # Nuevo nivel (Permite resultados negativos)
}
...
def generar_problema(dificultad):
    ...
    n1_max, n2_max, n3_max, _ = DIFICULTADES[dificultad]

    num1 = random.randint(1, n1_max)
    num2 = random.randint(1, n2_max)
    num3 = random.randint(1, n3_max)
    operador = random.choice(OPERACIONES)
    operador1 = random.choice(OPERACIONES)
    ...
    problema_str = f"{num1} {operador} {num2} {operador1} {num3}"
...
class TutorMatematicasApp:
    def __init__(self, master):
        ...
        if self.usuario_actual:
            ...
            self.actualizar_reloj() # Inicia la actualización del reloj
        else:
    ...
    def configurar_widgets(self):
        ...
        # --- Sección 1: Reloj y Separador ---
        self.lbl_reloj = tk.Label(self.master, text="", font=('Arial', 10), fg='#696969')
        self.lbl_reloj.pack(pady=(5, 0)) # Añadimos espacio arriba

        tk.Frame(self.master, height=1, bg='#D3D3D3').pack(fill='x', padx=10, pady=5)


        # --- Sección 2: Dificultad y Estadísticas ---
        ...
        # --- Sección 3: Problema y Respuesta ---
    ...
    def actualizar_reloj(self):
        """Actualiza la etiqueta del reloj con la fecha y hora actuales."""
        # Obtiene la fecha y hora actuales y las formatea
        ahora = datetime.now()
        tiempo_formateado = ahora.strftime("Fecha: %d/%m/%Y | Hora: %H:%M:%S")

        # Actualiza la etiqueta
        self.lbl_reloj.config(text=tiempo_formateado)

        # Llama a esta función de nuevo después de 1000 milisegundos (1 segundo)
    ...
    def comprobar_respuesta(self):
        ...
        try:
            ...
            # FIX: Usar messagebox en lugar de un Label temporal para errores de entrada
            messagebox.showerror("Error de Entrada", "Ingresa solo números enteros en el campo de respuesta.")
            return
        ...
        registro = {
            ...
            # Usamos la hora exacta registrada por el reloj
    ...
    def mostrar_lista_usuarios(self):
        ...
        # Botón para eliminar el historial del usuario seleccionado
        btn_eliminar = tk.Button(ventana_usuarios,
                                 text="🗑️ Eliminar Historial Seleccionado",
                                 command=lambda: self.eliminar_historial_usuario(listbox_usuarios.get(listbox_usuarios.curselection()[0]), ventana_usuarios) if listbox_usuarios.curselection() else messagebox.showerror("Error de Selección", "Selecciona un usuario primero."),
                                 font=('Arial', 12),
                                 bg='#D9534F',
                                 fg='white')
        btn_eliminar.pack(pady=(5, 15))
    ...
    def eliminar_historial_usuario(self, usuario_a_eliminar, ventana_lista):
        """Elimina el historial de un usuario del diccionario global y guarda."""

        if usuario_a_eliminar not in self.TODOS_LOS_DATOS:
            messagebox.showerror("Error", "Usuario no encontrado en el historial.")
            return

        confirmacion = messagebox.askyesno(
            "Confirmar Eliminación",
            f"¿Estás seguro de que deseas eliminar permanentemente todo el historial de '{usuario_a_eliminar}'?"
        )

        if confirmacion:
            del self.TODOS_LOS_DATOS[usuario_a_eliminar]
            guardar_historial(self.TODOS_LOS_DATOS)
            messagebox.showinfo("Éxito", f"El historial de {usuario_a_eliminar} ha sido eliminado.")

            # Cierra la ventana de lista de usuarios para forzar una recarga
            ventana_lista.destroy()

            # Si el usuario que está usando la app es el que se eliminó, forzamos cierre
